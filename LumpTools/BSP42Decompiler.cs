...
public class BSP42Decompiler {
	...
	public virtual Entities decompile() {
		...
		{
			// For each entity
			//Console.WriteLine("Entity " + i + ": " + mapFile[i]["classname"]);
		...
		{
			Console.WriteLine("Num simple corrected brushes: " + numSimpleCorrects);
			Console.WriteLine("Num advanced corrected brushes: " + numAdvancedCorrects);
			Console.WriteLine("Num good brushes: " + numGoodBrushes);
		}
	...
	private void  decompileBrush(Brush brush, int currentEntity)
	{
		...
		//Console.WriteLine(": " + numSides + " sides");
			...
			if ((currentFace.Flags[1] & ((sbyte) 1 << 0)) == 0)
			{
				...
				catch (System.IndexOutOfRangeException)
				{
					...
					catch (System.IndexOutOfRangeException f)
					{
						...
						Console.WriteLine("WARNING: BSP has error, references nonexistant plane " + currentSide.Plane + ", bad side " + (l) + " of brush " + numBrshs + " Entity " + currentEntity);
				...
				catch (System.IndexOutOfRangeException)
				{
					...
					Console.WriteLine("WARNING: Map referenced nonexistant material #" + currentFace.Material + ", using wld_lightmap instead!");
			...
			if (mapBrush.hasBadSide())
			{
				...
				if (mapBrush.hasGoodSide())
				{
					...
					if (Settings.calcVerts)
					{
						...
						{
							Console.WriteLine("WARNING: Brush vertex calculation failed on entity " + mapBrush.Entnum + " brush " + mapBrush.Brushnum + "");
						}
					}
				}
				else
				{
					...
					{
						Console.WriteLine("WARNING: Plane correct returned 0 triangles for entity " + mapBrush.Entnum + " brush " + mapBrush.Brushnum + "");
					}
				}
			}
			else
